 ...
 *  Redcollar.html
 */
define('vendors/droploadOld',function(require,exports,module){
    var $ = require('vendors/zepto'),
        com = require('lib/commonOld'),
        ...
        upload:function(parent,url,params,callback,numPerPage){
            ...
            $(parent).dropload({
                ...
                loadDownFn : function(me){
                    var isend = com.vm.isend;
                    console.log(isend);
                    if(isend && isend == '1'){
                        // alert.alertSecond('已经没有内容了~');
	                    AppInterface.toast('已经没有内容了~');
                        me.resetload();
                    }else{
                        self.loadMoreFun(url,params,callback,numPerPage);
                        me.resetload();
                    }   
                }
            });
        },
        loadMoreFun:function(url,params,callback,numPerPage){
            console.log(com.vm.pageNum);
            var isend = com.vm.isend;
            if(!isNaN(isend) && isend == 0){
                var pageNum = com.vm.pageNum;
                if(pageNum){

                    com.postData(url,params,callback,pageNum,numPerPage);
                }
            }else{
                if(!isNaN(isend) && isend == '1'){
                    // alert.alertSecond('已经没有内容了~');
	                AppInterface.toast('已经没有内容了~');
                }
            }
