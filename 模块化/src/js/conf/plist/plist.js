...
define('conf/plist/plist.js', function (require, exports, module) {
    ...
    function initControl() {
        ...
        // 如果列表有数据， 则隐藏没有数据图标
        if(vueObj.distributionItems.length > 0) {
            $('.common-nodata')[0].style.display = "none";
        } else {
            $('.common-nodata')[0].style.display = "block";
        }
        $('.common-prolist').show();
    }
    ...
    function initEvents() {
        $('#vue-plist').on('click', 'a.sale-btn-common', function (e) {
            ...
            AppInterface.notify('onProductClick', $(this).parent().attr('data-prdid'), $(this).parent().attr('data-shopid'));
        });
    }
    ...
    function render(data, index, $dom, isLoadMore, dataListKey) {
        var i = 0, itemList = dataListKey == 'classifyItems' ? data : data.items, len = itemList.length;
        if (!isLoadMore) {
        ...
        sale: function (id) {
            ...
            Ajax.update('/distribution/adddistribution', {vshopId: shopId, itemId: id}, function (data) {
                ...
                if (data.success) {
                    ...
                    arr[0].isSale = true;
                    var isSale = $('li[data-prdid="'+ id +'"]>a[class="sale-btn-common sale-btn"]');
                    isSale.text('取消分销');
                    isSale.attr('class', 'sale-btn-common sale-btn-no');
                }
            });
            return this;
        },
        ...
        nosale: function (id) {
            ...
            Ajax.update('/distribution/deldistribution', {vshopId: shopId, itemId: id}, function (data) {
                ...
                if (data.success) {
                    var arr = that.findItems(vueObj.distributionItems, 'id', id);
                    arr[0].isSale = false;
                    var noSale = $('li[data-prdid="'+ id +'"]>a[class="sale-btn-common sale-btn-no"]');
                    noSale.text('我要分销');
                    noSale.attr('class', 'sale-btn-common sale-btn');
                }
            });
            return this;
        },
        ...
        doSort: function () {
            ...
            Ajax.query('/distribution/getplist', {
                ...
                if (data.data && data.data.items) {
                    var i = 0, itemList = data.data.items, len = itemList.length;
                    if(len === 0) {
