define("conf/Operation/cont-jsonp3",function(require){
	...
	require('conf/Operation/m-share.js');
...
$(function(){
	...
	if(window.location.host == 'h5.dev.gomeplus.com'){
		obj.s10 = 123;
		otHost(obj);
		var oHostId = 'metaIds=72247,22185,67936,53421,53612,73984,37551,46125,27588,34365,123';//开发生产
		s5 = 73984,s6 = 37551,s7 = 46125,s8 =27588;
	}else if(window.location.host == 'h5.test.gomeplus.com' ){
		obj.s10 = 123;
		otHost(obj);
		var oHostId = 'metaIds=61595,27775,24867,51232,35917,31524,97451,16875,39146,74165,123'; // 测试环境
		s5 = 31524,s6 = 97451,s7 = 16875,s8 =39146;
	}else if(window.location.host == 'h5-pre.gomeplus.com'){
		obj.s10 = 12;
		otHost(obj);
		var oHostId = 'metaIds=61595,27775,24867,51232,35917,31524,97451,16875,39146,74165,12';//预生产
		s5 = 31524,s6 = 97451,s7 = 16875,s8 =39146;
	}else if(window.location.host == 'h5.gomeplus.com'){
		otHost(obj);
		var oHostId = 'metaIds=61595,27775,24867,51232,35917,31524,97451,16875,39146,74165,43'; //生产环境
		s5 = 31524,s6 = 97451,s7 = 16875,s8 =39146;
	}
	function otHost(obj){
		// 测试环境
	...
	var sectionMap = {
		...
		section1:function(data){
			...
			for(var i=1;i<oS1List.length;i++){		
			var oSection1ContR = 
		...
		section10:function(data){
			var oSectionShare = data.itemData;	
			for(var i=0;i<oSectionShare.length;i++){
				if(oSectionShare[i].activityLink.match("operation/indexthr")){	
					var shareTitle = oSectionShare[i].activityTitle;     		 //分享的title	
					var titleTpl = $('title').html();						     //页面title标题 								
					titleTpl = shareTitle;			
