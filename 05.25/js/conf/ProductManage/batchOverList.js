 ...
 *  <script src="__PUBLIC__/js/lithe.js"
     ...
     data-debug="true"
     data-main="conf/ProductManage/batchOverList.js">
     </script>
 */
define('conf/ProductManage/batchOverList.js',function(require,exports,module) {
    require('$');
    require('UI/tab.js');
    ...
    function init(){
        ...
        vueObj = new Vue({
            el:'#over-item-template',
            data:{
                items:window.items?window.items:[]
            }
        });
        $('#over-item-template').show();
    }
    ...
    function bindEvents(){
        ...
        $('.mshop-manageOver').on('click','ul.common-prolist>li.set',function(){
    ...
    function fetchData(pageNum){
        Ajax.query('/products/indextab',{type:0,shopId:shopId,pageNum:pageNum},function(data){
            if(pageNum == 1)
                vueObj.items = [];
